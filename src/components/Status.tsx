...
export const Status = ({
  ...
  const getStatusText = () => {
    ...
    const suffix = type === "increase" || type === "decrease" ? "%" : "";
  ...
  const getAriaLabel = () => {
    ...
    switch (type) {
      case "success":
        return `Sucesso: ${statusText}`;
      case "error":
        return `Erro: ${statusText}`;
      case "warning":
        return `Atenção: ${statusText}`;
      case "info":
        return `Informação: ${statusText}`;
      case "increase":
        return `Aumento de ${statusText}`;
      case "decrease":
        return `Diminuição de ${statusText}`;
      default:
        return statusText;
    }
  };
